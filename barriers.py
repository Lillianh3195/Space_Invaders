...
class Barrier:
  ...
  def update(self): 
    collisions = pg.sprite.groupcollide(self.alien_lasers.lasergroup(), self.pieces, True, True)  
    collisions = pg.sprite.groupcollide(self.ship_lasers.lasergroup(), self.pieces, True, True)  
    self.draw()
...
'''
# padding betw edge of screen, and spacing between barriers
# 4 rectangle blocks
# barrier has an x, y pos (arranges sprites)
# barrier constructs sprites, tells sprites to construct themselves, updates
# non-moving barrier
# how many rows and columns in your barrier
# barrier made up of sprites/pieces, which are in a sprite_group
# ask sprite_group if lasers collide with them (from alien and ship)

#barrierpiece class is to draw itself (don't really need this)
# barrierpiece has x, y pos, draw itself (rectangle)
'''